...
public class NewAtgFileAction extends CreateFileFromTemplateAction {
    ...
    protected boolean isAvailable(final DataContext dataContext) {
        ...
        if (project != null && view != null && view.getDirectories().length > 0) {
            available = Arrays.stream(view.getDirectories()).anyMatch(this::isUnderConfigDirectory);
        }
        return available;
    }

    private boolean isUnderConfigDirectory(PsiDirectory dir) {
        VirtualFile virtualFile = dir.getVirtualFile();
        //TODO fix ability create new ATG xml file only for config folder by info from facet perhaps
        return virtualFile.getPath().contains("config");
    }
